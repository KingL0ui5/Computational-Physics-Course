...
def ps4problem4():
    ...
    def metropolis_integration(f, f_prop, f_comparison, x_0, N=10000, kwrgs=None):
        ...
        f_samples_comparison = np.array(
            [f_comparison(x) for x in samples_afterburn])
    ...
    for i in sigma:
        I2d = metropolis_integration(
            func2d, gaussian, compare2d, [0.5, 0.5], kwrgs=i)
        I3d = metropolis_integration(func3d, gaussian, compare3d, [
                                     0.5, 0.5, 0.5], kwrgs=i)
        error3d.append(((4/3 * np.pi) - I3d)/(4/3 * np.pi))
        error2d.append((np.pi - I2d)/np.pi)
...
def ps4problem3():
    ...
    I, err = MC_integrate_nd(
        func3d, 3, [-0.5, -0.5, -0.5], [0.5, 0.5, 0.5], N=100000)
    print(f"Integral ≈ {I:.6f} ± {err:.6f}")
...
def problem3():
    def jacobi_inverse(A, b, max_step=100, stop_diff=1e-6):
        ...
        step = 0
        # This did not work originally, due to the arrays being initialised as integers not floats!
        x_k = np.zeros_like(b, dtype=float)
...
def problem6():
    ...
    plt.plot(time, B_prime_all[0, :],
             label='Interpolated Bx (KMH)', color='tab:blue')
    plt.ylabel('Bx [nT]')
    plt.legend()
    plt.grid(True)
    ...
    plt.plot(time, B_prime_all[1, :],
             label='Interpolated By (KMH)', color='tab:orange')
    plt.ylabel('By [nT]')
    plt.xlabel('Time [UTC]')
    plt.legend()
    plt.grid(True)
...
def problem6d():
    ...
    plt.figure(figsize=(12, 8))
    plt.suptitle(
        f'Interpolated (Jacobi) vs Measured Geomagnetic Field (N={N})')

    plt.subplot(2, 1, 1)
    plt.plot(time_sliced, df_KMH_sliced['Bx'],
             label='Measured Bx (KMH)', color='k')
    plt.plot(time_sliced, B[0, :], label='Interpolated Bx (Jacobi)',
    ...
    plt.subplot(2, 1, 2)
    plt.plot(time_sliced, df_KMH_sliced['By'],
             label='Measured By (KMH)', color='k')
    plt.plot(time_sliced, B[1, :], label='Interpolated By (Jacobi)',
